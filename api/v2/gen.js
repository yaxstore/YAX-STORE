...
async function gen(region, prefix) {
  ...
  const r = await s.post('https://100067.connect.garena.com/api/v2/oauth/guest:register', rp, {
    headers: {
  ...
  if (r.status !== 200 || r.data.code !== 0) throw new Error('reg fail');
  ...
  const t = await s.post('https://100067.connect.garena.com/api/v2/oauth/guest/token:grant', tp, {
    headers: {
  ...
  if (t.status !== 200 || t.data.code !== 0) throw new Error('tok fail');
...
module.exports = async (req, res) => {
  ...
  if (!accounts.length) {
    return res.status(500).json({ success:false, accounts:[], errors });
  }
